<script>
import HiddenNewsForm from './componentsNewsList/HiddenNewsForm.vue';
...
export default {
    ...
    methods: {
        expandNewsForm() {
            const hiddenBlock = this.$refs.hiddenNewsForm.$refs.hiddenBlock;

            console.log("Развертывание", hiddenBlock);
            ...
            if (hiddenBlock.classList.contains('hidden')) {
                ...
                hiddenBlock.addEventListener('animationend', function handleAnimationEnd() {
                    hiddenBlock.classList.add('hidden');
                    hiddenBlock.classList.remove('fade-out');
                    hiddenBlock.removeEventListener('animationend', handleAnimationEnd);
                });
            }
        },
    },
    mounted() {
        console.log('mounted');
        const textElement = this.$refs.shortDescription;
...
<template>
    <div class="history_news_cont_main">
        <div class="history_news_cont_main_row d-flex justify-content-between align-items-center">
            <div class="history_news_cont_main_row_id_cont d-flex justify-content-center align-items-center">
                <p class="history_news_cont_main_row_id">{{ index }}</p>
            </div>
            <div class="history_news_cont_main_row_title_cont d-flex justify-content-center align-items-center">
                <p class="history_news_cont_main_title">{{ title }}</p>
            </div>
            <div class="history_news_cont_main_row_text_cont">
                <p class="history_news_cont_main_row_text" ref="shortDescription">Киберспорт - это мир виртуальных соревнований, где игроки демонстрируют свое мастерство в различных компьютерных играх. Этот вид спорта стремительно набирает популярность во всем мире, привлекая миллионы зрителей и участников.</p>
            </div>
            <div class="history_news_cont_main_row_date_cont d-flex justify-content-center align-items-center">
                <p class="history_news_cont_main_row_date">21.03.2024</p>
            </div>
            <div class="history_news_cont_main_row_status_cont ">
                <p class="true">Опубликовано</p>
            </div>
            <div class="history_news_cont_main_row_button_cont">
                <button class="history_news_cont_main_row_button" @click="expandNewsForm">
                <svg xmlns="http://www.w3.org/2000/svg" width="17" height="10" viewBox="0 0 17 10" fill="none">
                    <path d="M16 1L8.5 8L0.999999 1" stroke="#787C87" stroke-width="2" stroke-linecap="round"/>
                </svg>
                </button>
            </div>
        </div>

        <HiddenNewsForm ref="hiddenNewsForm"/>
