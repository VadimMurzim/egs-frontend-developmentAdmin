...
export default {
    data() {
        return {
            wiredShowPreview: this.showImagePreview,
            wiredContactImageFile: Object.assign({}, this.imageFile),
            wiredContactImagePreview: this.imagePreview.slice(),
        };
    },
    methods: {
        handleFileUpload() {

            this.wiredContactImageFile = this.$refs.file.files[0];

            let reader = new FileReader();

            reader.addEventListener("load", function () {
                this.wiredShowPreview = true;
                this.wiredContactImagePreview = reader.result;
                
                console.log("Показать предпросмотр", this.wiredShowPreview);

                this.$emit("send-file", {
                  file: this.wiredContactImageFile,
                  imagePreview: this.wiredContactImagePreview,
                  show: this.wiredShowPreview,
                });

            }.bind(this), false)

            if( this.wiredContactImageFile) {
                if ( /\.(jpe?g|png)$/i.test(this.wiredContactImageFile.name)) {
                    reader.readAsDataURL(this.wiredContactImageFile)
                }
            }
        },
    },
    updated() {
      this.wiredContactImageFile = this.imageFile;
      this.wiredContactImagePreview = this.imagePreview;
      this.wiredShowPreview = this.showImagePreview;
...
<template>
        <label for="imageInput" class="craete_news_input_img">
          <img src="../../../../../assets/img/site/add_img.svg" alt="Add Image" />
          <p class="craete_news_input_img_h2">Добавить обложку</p>
          <div class="d-flex justify-content-center flex-column">
            <p class="craete_news_input_img_h3 d-flex justify-content-center align-items-center">
              Размер изображения — 2560x1600
            </p>
            <p class="craete_news_input_img_h3 d-flex justify-content-center align-items-center">
              Допустимые форматы: .png .jpg
            </p>
          </div>
          <input type="file" id="imageInput" ref="file" accept=".png,.jpg" v-on:change="handleFileUpload()" style="display: none;" />
          <img :src="wiredContactImagePreview" v-show="wiredShowPreview"
          width="150px"
          height="100px"/>
        </label>
</template>
